...
export class Zip {
    public static ProgressUpdateRate = 500;

    public static zip() {
    }
    public static unzipWithProgress(archive: string, destination: string, progressCallback: (progress: number) => void, overwrite?: boolean, password?: string): Promise<any> {
            ...
            try {
                ...
                const progressInterval = setInterval(() => {
                    if (monitor.getState() === ProgressMonitor.STATE_BUSY) {
                        progressCallback(monitor.getPercentDone());
                    } else {
                        // Done working
